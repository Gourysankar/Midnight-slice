...
export default function Checkout() {
  ...
  async function handlePay() {
    ...
    try {
      ...
      if (isSingle) {
        ingredientsToCheck = [singleOrder.pizzaBase, singleOrder.sauce, singleOrder.cheese, ...(singleOrder.veggies || []), ...(singleOrder.meat || [])].filter(Boolean)
      } else {
        cart.forEach(item => {
          ingredientsToCheck.push(item.pizzaBase || item.base, item.sauce, item.cheese, ...(item.veggies || []), ...(item.meat || []))
        })
        ingredientsToCheck = ingredientsToCheck.filter(Boolean)
      }

      if (ingredientsToCheck.length > 0) {
        const stockRes = await API.post('/admin/inventory/check-stock', { items: ingredientsToCheck })
        if (!stockRes.data.success) {
          addToast(stockRes.data.message || 'One or more items are out of stock', 'error')
          setPaying(false)
          return
        }
      }
